...
def parse_min_months(value):
    min_months = str(value or "1").strip()
    try:
        return min_months, Decimal(min_months.replace(",", ".") or "0")
    except Exception:
        return "1", Decimal("1")
...
def load_debtors_report(min_months_value):
    ...
    rows = [
        ...
        if debtor["debt_months"] is not None and debtor["debt_months"] > min_months_value
    ]
...
def create_app():
    ...
    def debtors():
        ...
        phones = []
        min_months, min_months_value = parse_min_months(request.args.get("months", "1"))

        try:
            rows, stats, phones = load_debtors_report(min_months_value)
        except Exception as exc:
            db_error = str(exc)
    ...
    def debtor_phones():
        min_months, min_months_value = parse_min_months(request.args.get("months", "1"))
        _rows, _stats, phones = load_debtors_report(min_months_value)
        body = "\n".join(phones)
        if body:
            body += "\n"
        return Response(
            ...
            headers={"Content-Disposition": f"attachment; filename=debtors-phones-{min_months}.txt"},
        )
